...
def load_articles(articles_dirname, gloves):
    ...
    """
    list_of_paths = filelist(articles_dirname)
    article_storage = []

    for file_path in list_of_paths:
        if file_path[-4:] =='.txt':
            file_name = '/'.join(file_path.split('/')[-2:])
            no_extention = ''.join(file_name).split('.')[0]
            file_text = get_text(file_path)
            title = file_text.split('\n')[0]
            file_text = words(' '.join(file_text.split('\n')[1:]))
            #assuming title only on first line
            centroid = doc2vec(file_text,gloves)
            article_storage.append([file_name,title,file_text,centroid])
...
def doc2vec(text, gloves):
    ...
    not in gloves.
    """
    centroid = np.zeros(shape=(1,300),dtype=float)
    word_count = len(text)
    for word in text:
        if word in gloves:
            #find vector sum of all words in document
            centroid = centroid + gloves[word]
    centroid = centroid / word_count
    return centroid

def distances(article, articles):
    """
    Compute the euclidean distance from article to every other
    article and return
    ...
    for a in articles:
        ...
        adistances.append((distance,a))
    return adistances


    # art_centroid = article[3]
    # for doc in articles:
    #     doc_centroid = doc[3]
    #     distance = dist.cdist(art_centroid, doc_centroid, 'euclidean')
    #     doc.append(distance)
    # return articles


def recommended(article2, articles2, n):
    """
    ...
    """
    doc_dist_list = distances(article2,articles2)
    sorted_list = sorted(doc_dist_list)
    n_list = sorted_list[:n]
    return n_list
